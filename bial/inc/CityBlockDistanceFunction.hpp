...
#ifndef BIALCITYBLOCKDISTANCEFUNCTION_H
#define BIALCITYBLOCKDISTANCEFUNCTION_H
...
namespace Bial {
  ...
  class CityBlockDistanceFunction : public DistanceFunction< CityBlockDistanceFunction > {
...
namespace Bial {
  ...
  template< class D >
  double CityBlockDistanceFunction::Distance( const D *src, const size_t dms ) {
    double dist = 0.0;
    for( size_t ftr = 0; ftr < dms; ++ftr ) {
      D src_ftr = ( src[ ftr ] > 0 ) ? src[ ftr ] : -src[ ftr ];
      dist += src_ftr;
    }
    return( dist );
  }
  ...
  template< class D >
  double CityBlockDistanceFunction::Distance( const D *src, const D *tgt, const size_t dms ) {
    double dist = 0.0;
    for( size_t ftr = 0; ftr < dms; ++ftr ) {
      D diff_ftr = ( src[ ftr ] > tgt[ ftr ] ) ? src[ ftr ] - tgt[ ftr ] : tgt[ ftr ] - src[ ftr ];
      dist += diff_ftr;
    }
    return( dist );
  }
...
#include "CityBlockDistanceFunction.cpp"
