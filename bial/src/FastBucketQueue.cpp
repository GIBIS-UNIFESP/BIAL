...
#ifndef BIALFASTBUCKETQUEUE_C
#define BIALFASTBUCKETQUEUE_C

#include "FastBucketQueue.hpp"

#if defined ( BIAL_EXPLICIT_LIB ) && ( BIAL_FastBucketQueue )
#define BIAL_EXPLICIT_FastBucketQueue
#endif

#if defined ( BIAL_EXPLICIT_FastBucketQueue ) || ( BIAL_IMPLICIT_BIN )

namespace Bial {

  FastBucketQueue::FastBucketQueue( size_t size, ldbl minimum_value, size_t n_buckets, bool increasing_order, 
                                    bool fifo_tie ) try :
    BucketQueue( size, increasing_order, fifo_tie ), minimum( minimum_value ),
      maximum( minimum_value + n_buckets - 1 ),
      current( increasing_order ? minimum_value : minimum_value + n_buckets - 1 ) {
  ...
  size_t FastBucketQueue::Index( ldbl value ) {
    return( static_cast< size_t >( value - minimum ) );
  }

  void FastBucketQueue::Insert( size_t idt, ldbl wgt ) {
    try {
  ...
  void FastBucketQueue::Remove( size_t idt, ldbl wgt ) {
    try {
  ...
  void FastBucketQueue::Update( size_t idt, ldbl cur_wgt, ldbl new_wgt ) {
    try {
      if( ( this->identity( idt ).state == BucketState::INSERTED ) || 
          ( this->identity( idt ).state == BucketState::UPDATED ) ) {
