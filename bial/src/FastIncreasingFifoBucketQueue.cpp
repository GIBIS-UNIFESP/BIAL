...
#ifndef BIALFASTINCREASINGFIFOBUCKETQUEUE_C
#define BIALFASTINCREASINGFIFOBUCKETQUEUE_C

#include "FastIncreasingFifoBucketQueue.hpp"

#if defined ( BIAL_EXPLICIT_LIB ) && ( BIAL_FastIncreasingFifoBucketQueue )
#define BIAL_EXPLICIT_FastIncreasingFifoBucketQueue
#endif

#if defined ( BIAL_EXPLICIT_FastIncreasingFifoBucketQueue ) || ( BIAL_IMPLICIT_BIN )

namespace Bial {

  FastIncreasingFifoBucketQueue::FastIncreasingFifoBucketQueue( size_t size, ldbl minimum_value, size_t n_buckets ) try
    : BucketQueue( size, true, true ), minimum( minimum_value ), maximum( minimum_value + n_buckets - 1 ),
  ...
  size_t FastIncreasingFifoBucketQueue::Index( ldbl value ) {
    return( static_cast< size_t >( value - minimum ) );
  }

  void FastIncreasingFifoBucketQueue::Insert( size_t idt, ldbl wgt ) {
    try {
      ...
      IF_DEBUG( ( this->identity( idt ).state == BucketState::INSERTED ) || 
  ...
  void FastIncreasingFifoBucketQueue::Remove( size_t idt, ldbl wgt ) {
    try {
  ...
  void FastIncreasingFifoBucketQueue::Update( size_t idt, ldbl cur_wgt, ldbl new_wgt ) {
    try {
