...
float UNSER::StandardDeviation( float contrast, float correlation ){ return(sqrt(correlation+contrast)); }


UNSER::UNSER(FeatureDetector<int> *Fd) : UNSER ( Fd->Run() ){ }

UNSER::UNSER(Vector<tuple<Image<int>,Image<int>>> detected) : FeatureExtractor< int , int >(detected){ }

void UNSER::SetParameters( ParameterInterpreter* interpreter ){ }

string UNSER::GetParameters( ParameterInterpreter* interpreter )
{
  return "";
}
