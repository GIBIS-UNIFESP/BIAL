...
#ifndef BIALFASTDECREASINGFIFOBUCKETQUEUE_C
#define BIALFASTDECREASINGFIFOBUCKETQUEUE_C

#include "FastDecreasingFifoBucketQueue.hpp"

#if defined ( BIAL_EXPLICIT_LIB ) && ( BIAL_FastDecreasingFifoBucketQueue )
#define BIAL_EXPLICIT_FastDecreasingFifoBucketQueue
#endif

#if defined ( BIAL_EXPLICIT_FastDecreasingFifoBucketQueue ) || ( BIAL_IMPLICIT_BIN )

namespace Bial {

  FastDecreasingFifoBucketQueue::FastDecreasingFifoBucketQueue( size_t size, ldbl minimum_value, size_t n_buckets ) try 
    : BucketQueue( size, false, true ), minimum( minimum_value ), maximum( minimum_value + n_buckets - 1 ),
      current( minimum_value + n_buckets - 1 ) {
  ...
  size_t FastDecreasingFifoBucketQueue::Index( ldbl value ) {
    return( static_cast< size_t >( value - minimum ) );
  }

  void FastDecreasingFifoBucketQueue::Insert( size_t idt, ldbl wgt ) {
    try {
  ...
  size_t FastDecreasingFifoBucketQueue::Remove( ) {
    try {
  ...
  void FastDecreasingFifoBucketQueue::Remove( size_t idt, ldbl wgt ) {
    try {
  ...
  void FastDecreasingFifoBucketQueue::Update( size_t idt, ldbl cur_wgt, ldbl new_wgt ) {
    try {
      if( ( this->identity( idt ).state == BucketState::INSERTED ) || 
          ( this->identity( idt ).state == BucketState::UPDATED ) ) {
