...
#ifndef BIALFASTDECREASINGLIFOBUCKETQUEUE_C
#define BIALFASTDECREASINGLIFOBUCKETQUEUE_C

#include "FastDecreasingLifoBucketQueue.hpp"

#if defined ( BIAL_EXPLICIT_LIB ) && ( BIAL_FastDecreasingLifoBucketQueue )
#define BIAL_EXPLICIT_FastDecreasingLifoBucketQueue
#endif

#if defined ( BIAL_EXPLICIT_FastDecreasingLifoBucketQueue ) || ( BIAL_IMPLICIT_BIN )

namespace Bial {

  FastDecreasingLifoBucketQueue::FastDecreasingLifoBucketQueue( size_t size, ldbl minimum_value, size_t n_buckets ) try 
    : BucketQueue( size, false, false ), minimum( minimum_value ), maximum( minimum_value + n_buckets - 1 ), 
  ...
  size_t FastDecreasingLifoBucketQueue::Index( ldbl value ) {
    return( static_cast< size_t >( value - minimum ) );
  }

  void FastDecreasingLifoBucketQueue::Insert( size_t idt, ldbl wgt ) {
    try {
  ...
  void FastDecreasingLifoBucketQueue::Remove( size_t idt, ldbl wgt ) {
    try {
  ...
  void FastDecreasingLifoBucketQueue::Update( size_t idt, ldbl cur_wgt, ldbl new_wgt ) {
    try {
      if( ( this->identity( idt ).state == BucketState::INSERTED ) || 
          ( this->identity( idt ).state == BucketState::UPDATED ) ) {
