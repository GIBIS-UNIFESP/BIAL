...
int main( int argc, char **argv ) {
  if( ( argc < 5 ) || ( argc == 6 ) || ( argc == 8 ) ) {
    ...
    cout <<
    "\tcsf_min_scl, csf_max_scl: Default values = 0.1, 0.3. Suggested values: 0.1 <= csf_min_scl <= csf_max_scl <= 0.9."
         << endl;
    cout <<
    "\tgm_min_scl, gm_max_scl: Default values = 0.3, 0.5. Suggested values: 0.1 <= gm_min_scl <= gm_max_scl <= 0.9." <<
    endl;
    exit( 1 );
  }
  ...
  float gm_min_scl = 0.3f;
  float gm_max_scl = 0.5f;
  if( argc > 7 ) {
