...
int main( int argc, char **argv ) {
  ...
  for( size_t pxl = 0; pxl < lbl.size( ); ++pxl ) {
    int src_lbl = lbl( pxl );
    int adj_lbl = -1;
    for( AdjacencyIterator itr = begin( adj, lbl, pxl ); *itr < lbl.size( ); ++itr ) {
      size_t adj_pxl = *itr;
  ...
  for( size_t src_vtx = intersection.size( ) - 1; src_vtx < intersection.size( ); --src_vtx ) {
    ...
    Vector< int > src_lbl;
    bool valid = true;
    for( AdjacencyIterator itr = begin( adj, lbl, lbl.Position( intersection[ src_vtx ] ) ); *itr < lbl.size( );
         ++itr ) {
      bool insert = true;
      int new_lbl = lbl[ *itr ];
    ...
    for( size_t tgt_vtx = src_vtx + 1; tgt_vtx < intersection.size( ); ++tgt_vtx ) {
      ...
      Vector< int > tgt_lbl;
      for( AdjacencyIterator itr = begin( adj, lbl, lbl.Position( intersection[ tgt_vtx ] ) ); *itr < lbl.size( );
           ++itr ) {
        bool insert = true;
        int new_lbl = lbl[ *itr ];
  ...
  for( size_t src_vtx = vertex.size( ) - 2; src_vtx < vertex.size( ); --src_vtx ) {
    COMMENT( "Getting labels of the source element.", 3 );
    Vector< int > src_lbl;
    for( AdjacencyIterator itr = begin( adj, lbl, lbl.Position( vertex[ src_vtx ] ) ); *itr < lbl.size( ); ++itr ) {
      bool insert = true;
      int new_lbl = lbl[ *itr ];
      for( size_t sl = 0; sl < src_lbl.size( ); ++sl ) {
        if( src_lbl[ sl ] == new_lbl ) {
    ...
    for( size_t tgt_vtx = src_vtx + 1; tgt_vtx < vertex.size( ); ++tgt_vtx ) {
      ...
      Vector< int > tgt_lbl;
      for( AdjacencyIterator itr = begin( adj, lbl, lbl.Position( vertex[ tgt_vtx ] ) ); *itr < lbl.size( ); ++itr ) {
        bool insert = true;
        int new_lbl = lbl[ *itr ];
        for( size_t tl = 0; tl < tgt_lbl.size( ); ++tl ) {
          if( tgt_lbl[ tl ] == new_lbl ) {
