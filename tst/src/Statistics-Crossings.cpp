...
template< class D >
int Crossings( const Image< D > &src, const Image< D > &tgt, float dist ) {
  try {
    ...
    if( img_dims == 2 ) {
      ...
      return( RunContour( src, tgt_lbl, src_dst, dist ) );
    }
    else {
      COMMENT( "3D image.", 0 );
	...
	crossings += RunContour( src_slice, tgt_lbl_slice, src_dst_slice, dist );
      }
      return( crossings );
    }
  }
...
int main( int argc, char **argv ) {
  try {
    ...
    std::cout << Crossings( src, tgt, dist ) << std::endl;
    return( 0 );
  }
  catch( std::bad_alloc &e ) {
    std::string msg( e.what( ) + std::string( "\n" ) + BIAL_ERROR( "Memory allocation error." ) );
